...
try {
	...
	boolean hpcclusterTmp=	 CustomKeywords.'customWait.WaitForElement.WaitForelementPresent'(findTestObject('Object Repository/AppComposer/hpcluster_temp'), 20, extentTest, 'hpcCluster')
	
		
	if(hpcclusterTmp) {
		extentTest.log(Status.PASS, 'Click on hpccluster')
	...
	switch (userChoice) {
		...
		case 'Delete App':
			...
			WebUI.waitForElementPresent(findTestObject('AppComposer/Appcomposer'), 10)
		...
		//WebUI.verifyElementPresent(findTestObject('null'), 3)
			...
			result = WebUI.waitForElementPresent(findTestObject('Object Repository/AppComposer/JobStatus'), 10)
