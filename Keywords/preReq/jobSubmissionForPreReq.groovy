...
public class jobSubmissionForPreReq {
	...
	def JSPreReq(String newFP , String AppName) {
		...
		if(App.equals('radioss-smp')) {
			...
			def f2 = RunConfiguration.getProjectDir() + '/Upload/LAME_EQUERRE_0001.rad'
		...
		def submitBtn1=(new customWait.WaitForElement()).WaitForelementPresent (findTestObject('JobSubmissionForm/button_Submit_Job'), 10)
		if(submitBtn1) {
			WebUI.click(findTestObject('JobSubmissionForm/button_Submit_Job'))
		}
		WebUI.waitForElementPresent(findTestObject('Notificactions/Notification_JobSubmission'), 5)
