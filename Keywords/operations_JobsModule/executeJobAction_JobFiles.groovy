		...
		switch (Action) {
			...
			case 'Delete':
				...
				if (TestCaseName.contains("No")) {
					...
					isElementPresent=WebUI.verifyElementPresent(newFileObj, 3, FailureHandling.CONTINUE_ON_FAILURE)
					if(isElementPresent) {
			...
			case 'Rename':
				...
				TestObject newFileObj1 = WebUI.modifyObjectProperty(findTestObject('FilesPage/RowItem_File_ListView'), 'data-automation-id', 'equals',
						Renameto, true)
				def isElemenetPresent=WebUI.waitForElementPresent(newFileObj1, 10,FailureHandling.CONTINUE_ON_FAILURE)
