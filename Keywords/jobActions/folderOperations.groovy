	...
	def executeFileOperations(String Operation,String TestCaseName ,extentTest) {
		...
		switch (Operation) {
			...
			case 'Rename':
				...
				WebUI.delay(2)
				WebUI.waitForElementPresent(findTestObject('FilesPage/NewFile_input'), 3)
				if(TestCaseName.contains('tile view')) {
					...
					TestObject  newFileObj = WebUI.modifyObjectProperty(findTestObject('Object Repository/FilesPage/FolderRowItem_TileView'), 'title', 'equals',
							Renameto, true)
					def isElemenetPresent=WebUI.waitForElementPresent(findTestObject('Object Repository/Notificactions/Notification_CopyFile'), 10,FailureHandling.CONTINUE_ON_FAILURE)
					if(isElemenetPresent){
					...
					TestObject newFileObj = WebUI.modifyObjectProperty(findTestObject('Object Repository/FilesPage/FolderRowItem_ListView'), 'title', 'equals',
							Renameto, true)
					def isElemenetPresent=WebUI.waitForElementPresent(newFileObj, 10,FailureHandling.CONTINUE_ON_FAILURE)
					if(isElemenetPresent){
			...
			//Verify notification
				...
				if (TestCaseName.contains('tile view')) {
					...
					def isElemenetPresent=WebUI.waitForElementPresent(findTestObject('FilesPage/RowItem_CompressedFolder_TileView'), 10, FailureHandling.CONTINUE_ON_FAILURE)
					println  "RowItem_CompressedFolder_TileView - "+isElemenetPresent
					result=isElemenetPresent
				}
				else {
					def isElemenetPresent=WebUI.waitForElementPresent(findTestObject('FilesPage/RowItem_CompressedFolder_ListView'), 10, FailureHandling.CONTINUE_ON_FAILURE)
