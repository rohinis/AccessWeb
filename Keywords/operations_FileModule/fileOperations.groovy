...
public class fileOperations {
	...
	@Keyword
	def executeFileOperations(String TestOperation,String TestCaseName ,extentTest,katalonWebDriver) {
		...
		switch (TestOperation) {
			...
			case 'Copy':
				...
				if (TestCaseName.contains('tile view')) {
					...
					WebUI.delay(2)
					def isElemenetPresent=WebUI.waitForElementPresent(findTestObject('FilesPage/Label_FolderEmpty'),5,FailureHandling.CONTINUE_ON_FAILURE)
				...
				def isFilePresent=WebUI.waitForElementPresent(newFileObj, 10,FailureHandling.CONTINUE_ON_FAILURE)
				if(isFilePresent){
			...
			case 'Cut':
				...
				def isElemenetPresent=WebUI.waitForElementPresent(findTestObject('FilesPage/Label_FolderEmpty'), 10,FailureHandling.CONTINUE_ON_FAILURE)
			...
			case 'Rename':
				...
				WebUI.waitForElementPresent(renameTextBxObj, 3)
				...
				WebUI.delay(2)
				def isElemenetPresent=WebUI.waitForElementPresent(newFileObjVerify, 3,FailureHandling.CONTINUE_ON_FAILURE)
				if(isElemenetPresent){
					extentTest.log(Status.PASS, "Renamed file - "+Renameto + " is listed ")
					result=true
				}
				else {
					extentTest.log(Status.PASS, "Renamed file - "+Renameto + " is not listed ")
					result=false
				}
			...
			case 'Compress':
				...
				if(TestCaseName.contains('tile view')) {
					msg= 'The selected item is compressed successfully to '+GlobalVariable.G_StageArea+'/'+GlobalVariable.G_userName+'/FilesModule/FileOps/'+'ToCompress_TV_archive'
				}
				else {
					msg= 'The selected item is compressed successfully to '+GlobalVariable.G_StageArea+'/'+GlobalVariable.G_userName+'/FilesModule/FileOps/'+'ToCompress_LV_archive'
				}
				println("********************************************")
				println (msg)
				println("********************************************")
				if	(WebUI.waitForElementPresent(findTestObject('Object Repository/GenericObjects/Notification-Popup'), 5))
				{
					...
					{
						extentTest.log(Status.FAIL,"Notification Msg - "+notificationMsg)
					}
				}
				else
				{
				...
				if (TestCaseName.contains('tile view')) {
					...
					def isElemenetPresent=WebUI.waitForElementPresent(findTestObject('FilesPage/RowItem_CompressedFile_TileView'), 10, FailureHandling.CONTINUE_ON_FAILURE)
				}
				else {
					def isElemenetPresent=WebUI.waitForElementPresent(findTestObject('FilesPage/RowItem_CompressedFile_ListView'), 10, FailureHandling.CONTINUE_ON_FAILURE)
				}
				WebUI.delay(1)
			...
			case 'Delete':
				...
				println("isDelFilePresent" + isDelFilePresent)
			//isDelFilePresent=WebUI.waitForElementPresent(newFileObj, 3,FailureHandling.CONTINUE_ON_FAILURE)

				isDelFilePresent=(new customWait.WaitForElement()).WaitForelementPresent(newFileObj, 5,extentTest, 'DEleted File')

				println("isDelFilePresent" + isDelFilePresent)

				if(isDelFilePresent) {
					extentTest.log(Status.FAIL, 'Failed to delete  as per the test case')
					result=false
				}
				else {
