	...
	def executeFileOperations(String TestOperation,String TestCaseName ,extentTest) {
		...
		switch (TestOperation) {
			...
			case 'Copy':
				...
				if (result) {
					extentTest.log(LogStatus.PASS, fileToCheck + '- Copied in ToPaste and verified notification')
			...
			case 'Cut':
				...
				if (result) {
					extentTest.log(LogStatus.PASS, fileToCheck + '- Moved in ToPaste and verified notification')
					extentTest.log(LogStatus.PASS, ('Notification with msg - "' + msg) + '" is listed')
				}
				else {
					extentTest.log(LogStatus.PASS, fileToCheck + ' - Not pasted')
			...
			case 'Rename':
				...
				WebUI.delay(2)
				if (TestCaseName.contains('Job Submission')) {
					/*	oriFileName='ToRename.txt'
				 Renameto='RenamedFile.txt'
				 renameTextBxObj = WebUI.modifyObjectProperty(findTestObject('FilesPage/NewFile_input'), 'value', 'equals', 'ToRename.txt', true)
				 newFileObjVerify = WebUI.modifyObjectProperty(findTestObject('FilesPage/RowItem_File_ListView'), 'title', 'equals',Renameto, true)
				 extentTest.log(LogStatus.PASS,'orifilename - '+oriFileName)
				 */
				...
				result=WebUI.waitForElementPresent(newFileObjVerify, 10, FailureHandling.CONTINUE_ON_FAILURE)
				WebUI.click(findTestObject('Landing_Page/Btn_Notifiction'))
				extentTest.log(LogStatus.PASS, 'Click on Notification button to open Notification Panel')
				WebUI.delay(2)
				result = WebUI.verifyElementPresent(findTestObject('Notificactions/Notification_Rename'),5)
				if (result) {
					extentTest.log(LogStatus.PASS, 'Verifed '+oriFileName+' renamed to '+Renameto+' and verified notification')
					extentTest.log(LogStatus.PASS, ('Notification with msg - "' + msg) + '" is listed')
				}
				else {
					extentTest.log(LogStatus.PASS, Renameto + ' - Not pasted')
					extentTest.log(LogStatus.FAIL)
				}
				WebUI.click(findTestObject('Landing_Page/Btn_Notifiction'))
				extentTest.log(LogStatus.PASS, 'Click on Notification button to close Notification Panel')
