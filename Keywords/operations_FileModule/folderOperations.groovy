	...
	def executeFolderOperations(String Operation,String TestCaseName ,extentTest) {
		...
		switch (Operation) {
			...
			case 'Copy':
				...
				def isFilePresent=WebUI.waitForElementPresent(newFolderObj, 10,FailureHandling.CONTINUE_ON_FAILURE)
			...
			case 'Rename':
				...
				WebUI.delay(2)
				def isElemenetPresent=WebUI.waitForElementPresent(newFolderObjVerify, 3,FailureHandling.CONTINUE_ON_FAILURE)
				if(isElemenetPresent){
			...
			case 'Compress':
				...
				if (TestCaseName.contains('tile view')) {
					...
					def isElemenetPresent=WebUI.waitForElementPresent(findTestObject('FilesPage/RowItem_CompressedFile_ListView'), 15, FailureHandling.CONTINUE_ON_FAILURE)
				}
				WebUI.delay(2)
